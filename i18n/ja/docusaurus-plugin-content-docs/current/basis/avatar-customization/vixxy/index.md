...
別のメッシュが表示されているかどうかに基づいて、コントロールの値を上書きすることを選択できます。

いくつかの例を挙げます：

### 他のメッシュによるメッシュの非表示

ブラジャーとTシャツの両方が個別に切り替え可能であるが、Tシャツが表示されているときはブラジャーを隠す必要がある場合：

- ブラジャーの *Vixxy Control* で *"+ Add Mesh Visibility condition"* をクリックします。
- *Mesh Visibility* コンポーネントで：
  - ブラジャーを OFF にするために、Tシャツの **Mesh**（コントロールでは**ない**）を指定します（その選択肢に対応する値を使用します）。
  - *Otherwise* セクションでは、メニューの値が有効になるように、値を *Do Not Override* のままにします。

### 複数の他のメッシュによるボディのブレンドシェイプの縮小

靴下や靴のいずれかが表示されているときに、ボディメッシュの一部をブレンドシェイプで隠す必要がある場合：

- ボディメッシュのブレンドシェイプを制御するために使用する、新しい *Vixxy Control* を作成します。
- *Select...* ボタンを使用して *Mesh Visibility* を選択します。
- *Mesh Visibility* コンポーネントで：
  - ボディの一部を隠すべき靴下と靴の **Mesh**（コントロールでは**ない**）を指定し、メッシュが表示されているときの選択肢に対応する値を選択します。
  - *Otherwise* セクションで、ドロップダウンを使用して *Override Value* を選択し、メッシュが表示されていないときの選択肢に対応する値を選択します。
- ボディメッシュのブレンドシェイプを変更します（下のセクションを参照）。
...
::::warning
この機能は広範囲にテストされていないことに注意してください。このドキュメントは2026年9月19日に追加されましたが、2026年5月21日以降動作しているはずです。
::::

外部プログラムからコントロールを変更できる場合があります。ルールは以下の通りです：

- *Additional settings* で、`Example` などのアドレスを手動で設定します。
- 外部プログラムは、ポート 9000 で `/avatar/parameters/Example` というアドレスの OSC パケットを送信します。
  - `bool` 値ではなく、`float` 値を送信してください。
  - OSC 経由で `bool` 値を送信しても、効果がある場合とない場合があります。
- *Vixxy Control* コンポーネントで、サポートしたい最小値と最大値の選択肢が含まれるように選択肢を構成します。
  - 例えば、0 と 1、-1 と 1、または 40 と 200 などです。
- 最小値と最大値はどのような範囲でも構いませんが、ネットワーク経由では値は 255 個の異なる値に量子化されます。
  - 0.0 から 1.0 の間の値の場合、ステップは約 0.004（0.4%）です。
  - -1.0 から 1.0 の間の値の場合、ステップは約 0.008（0.8%）です。
  - -1.0、1.0、および 0.0 の値は、量子化後も同じ値であることが保証されます。
