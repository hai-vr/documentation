...
ライトボリュームなしでテストしたい場合は、2 つのオプションがあります：
- *Lightbox Scene* フィールドで、`LightboxLightVolumes` シーンの代わりに `Lightbox` というシーンを選択する。
- **または**、Lightbox Viewer を有効にした後、Lightbox シーン内を確認してください。名前に `NoLV` が含まれる休止中のライトボックスがありますが、これらは EditorOnly タグが付いているためレンダリングされません。これらのライトボックスから EditorOnly タグを削除すると、ライトボリュームありと共になしを並べてプレビューできるようになります。
